    ...
    public function AsdecodefilterBy($table, $req)
    {
        if (isset($_GET)) {
            if (isset($_GET['search_by_keyword']) && $_GET['search_by_keyword'] !== '') {
                ...
                if (isset($_GET['period_from']) && isset($_GET['period_to']) && $table == 'courses') {
                    $req = $req->whereBetween('date_chargement', [date('Y-m-d', strtotime($_GET['period_from'])), date('Y-m-d', strtotime($_GET['period_to']))]);
                } else if (isset($_GET['period_from']) && isset($_GET['period_to'])) {
                    $req = $req->whereBetween('created_at', [date('Y-m-d', strtotime($_GET['period_from'])), date('Y-m-d', strtotime($_GET['period_to']))]);
                }
