        ...
        <template #content>
            ...
            <div style="overflow-y: scroll; max-height: 90%;">
                <list-paginated url="/api/courses?etat=Terminé" v-if="tab === 'Terminé'">
                    <template v-slot:list_content = courses>
                        <ion-list v-for="course in courses.datas" :key="course.id" >
                            <ion-item @click="openModalTerminer(course)">
                              <div class="btn">
                                <div class="d-flex align-items-center row">
                                    <div class="col-3">
                                        <img src="../../assets/default_user.png" class="w-100 rounded-circle">
                            ...
                            <ion-modal ref="map" :trigger="'opem-map-' + course.id">
                                <ion-content>
                                  <ion-toolbar color="light">
                                    ...
                                    <ion-buttons slot="end">
                                      <ion-button @click="dismiss('map')"><span class="badge badge-secondary mx-2">x</span>fermer</ion-button>
                                    </ion-buttons>
                                  </ion-toolbar>
                                  <p>{{ course.id }}</p>
                                </ion-content>
          ...
          <ion-modal :is-open="map">
            <ion-header>
                <ion-toolbar color="primary" id="toolbar">
                  <ion-buttons slot="end">
                    <ion-button @click="closeModal()" :strong="true"><span class="badge badge-secondary mx-2">x</span>fermer</ion-button>
                  </ion-buttons>
                </ion-toolbar>
            </ion-header>
            <ion-content>
                <CourseShow :data="course"></CourseShow>
            </ion-content>
          </ion-modal>


          <ion-modal :is-open="mapTerminer">
            <ion-header>
                <ion-toolbar color="primary" id="toolbar">
                  <ion-buttons slot="end">
                    <ion-button @click="closeModalTerminer()" :strong="true"><span class="badge badge-secondary mx-2">x</span>fermer</ion-button>
                  </ion-buttons>
                </ion-toolbar>
            </ion-header>
            <ion-content>
                <CourseTerminer :data="course"></CourseTerminer>
            </ion-content>
          </ion-modal>
        </template>
...
import CourseTerminer from '@/views/courses/CourseTerminer.vue'

export default defineComponent({
    components: {IonSegment, IonSegmentButton, IonFab, IonFabButton, toastController, CourseShow, CourseTerminer},
    data() {
        return {
            isLoading: true,
            first: 0,
    ...
    methods: {
        ...
        openModalTerminer(data){
            this.mapTerminer = true
            this.course = data
        },
        closeModalTerminer(){
            this.mapTerminer = false
        },
        goMap(course, camion){
            // if (course.etat == 'En cours') {
            //     this.openMap(camion)
